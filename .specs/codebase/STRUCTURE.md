...
- `engine` — cadence execution. Holds the pure `LeadJourney` state machine (`core/domain/`), `EntryTrigger` CRUD, ingestion (`StartJourneyUseCase` + the public `crm-webhook.controller.ts`), and the dispatcher: `core/services/journey-dispatcher.ts` (row-locked dispatch in a transaction) + `cadence-action-executor.ts`, `core/poller/journey-poller.ts` (in-process `setInterval`), pure `core/domain/next-step.ts` + `jitter.ts`, and the `lead`/`lead-journey`/`touch-attempt` repositories (`persistence/transaction.ts` types the tx executor). Inbound reply is the `meta-webhook.controller.ts` (`@Public`) + `MarkReplyUseCase` (row-locked `→ replied` + `onReply`).
