...
- `engine` — cadence execution. Today holds the pure `LeadJourney` state machine (`core/domain/`: `LeadJourneyStatus`, `JourneyEvent`, `transition()`) and `EntryTrigger` CRUD (pipeline+stage → cadence). The scheduler/dispatcher, ingestion, inbound reply, and the `Lead`/`LeadJourney`/`TouchAttempt` tables join in later slices.
