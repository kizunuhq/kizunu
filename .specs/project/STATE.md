...
- `Template` (feature `005`, `modules/cadence/`) is a workspace-owned HSM reference (`providerTemplateName`, `language`, `variables[]`) with CRUD; unique name per workspace. The `cadence` module gains the `Cadence`/`Step` aggregate + `EntryTrigger` in feature `006`; `Lead` is mirrored during ingestion in the engine slice (not user-CRUD).
