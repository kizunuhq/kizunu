...
- The `engine` module (feature `007`, `modules/engine/`) holds the pure `LeadJourney` state machine (`core/domain/lead-journey-transition.ts`: a transition-table `Record`, not a switch — the D1 core, exhaustively tested) and `EntryTrigger` CRUD (one cadence per connector account + stage; validates both references in the workspace). `LeadJourneyStatus`/`JourneyEvent` are derived const objects (ADR 002); the `lead_journeys` pgEnum will conform via `Assert<Equal>` when added. Still to come: ingestion (creates `Lead` + `LeadJourney` from a `lead.stage_entered` event via the EntryTrigger), the scheduler/dispatcher (poller + `SELECT … FOR UPDATE` + `TouchAttempt`), and inbound reply.
