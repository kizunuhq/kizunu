...
export function createDrainPipeline<T = unknown>(
  ...
  if (batchSize <= 0 || !Number.isFinite(batchSize)) {
    throw new Error(
      `[evlog/pipeline] batch.size must be a positive finite number, got: ${batchSize}`,
    )
  }
  if (intervalMs <= 0 || !Number.isFinite(intervalMs)) {
    throw new Error(
      `[evlog/pipeline] batch.intervalMs must be a positive finite number, got: ${intervalMs}`,
    )
  }
  if (maxBufferSize <= 0 || !Number.isFinite(maxBufferSize)) {
    throw new Error(
      `[evlog/pipeline] maxBufferSize must be a positive finite number, got: ${maxBufferSize}`,
    )
  }
  if (maxAttempts <= 0 || !Number.isFinite(maxAttempts)) {
    throw new Error(
      `[evlog/pipeline] retry.maxAttempts must be a positive finite number, got: ${maxAttempts}`,
    )
  }
  if (initialDelayMs < 0 || !Number.isFinite(initialDelayMs)) {
    throw new Error(
      `[evlog/pipeline] retry.initialDelayMs must be a non-negative finite number, got: ${initialDelayMs}`,
    )
  }
  if (maxDelayMs < 0 || !Number.isFinite(maxDelayMs)) {
    throw new Error(
      `[evlog/pipeline] retry.maxDelayMs must be a non-negative finite number, got: ${maxDelayMs}`,
    )
  }
