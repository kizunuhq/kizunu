...
export function initLogger(config: LoggerConfig = {}): void {
  ...
  if (globalSilent && !hasAnyDrain && !config._suppressDrainWarning) {
    console.warn(
      '[evlog] silent mode is enabled but no drain is configured. Events will be built and sampled but not output anywhere. Set a drain via initLogger({ drain }) or a framework hook (evlog:drain).',
    )
  }
}
...
function shouldSample(level: LogLevel): boolean {
  ...
  const percentage = level === 'error' && rates.error === undefined ? 100 : (rates[level] ?? 100)
...
export function shouldKeep(ctx: TailSamplingContext): boolean {
  ...
  return keep.some((condition) => {
    ...
    if (
      ...
      ctx.duration >= condition.duration
    ) {
      return true
    }
...
function emitWideEvent(
  ...
  if (ownsEvent) {
    ...
    if (globalEnv.version !== undefined && event.version === undefined)
      event.version = globalEnv.version
    if (globalEnv.commitHash !== undefined && event.commitHash === undefined)
      event.commitHash = globalEnv.commitHash
    if (globalEnv.region !== undefined && event.region === undefined)
      event.region = globalEnv.region
...
function emitTaggedLog(level: LogLevel, tag: string, message: string): void {
  ...
  if (globalPretty && !globalSilent) {
    ...
    if (isBrowser()) {
      ...
      const timestamp = isoNow().slice(11, 23)
      console.log(
        `${colors.dim}${timestamp}${colors.reset} ${color}[${tag}]${colors.reset} ${message}`,
      )
    }
...
function buildAIEntries(ai: Record<string, unknown>): TreeEntry[] {
  ...
  // Steps
  const stepsUsage = Array.isArray(ai.stepsUsage) ? ai.stepsUsage.filter(isPlainObject) : undefined
...
function prettyPrintWideEvent(event: Record<string, unknown>): void {
  ...
  if (browser) {
    ...
    parts.push(
      `%c${ts}%c %c${levelLabel.toUpperCase()}%c %c[${escapeFormatString(String(service))}]%c`,
    )
    styles.push(
      cssColors.dim,
      cssColors.reset,
    ...
    parts.push(
      `${colors.dim}${ts}${colors.reset} ${lc}${levelLabel.toUpperCase()}${colors.reset} ${colors.cyan}[${service}]${colors.reset}`,
    )
  }
  ...
  if (rest.method && rest.path) {
    parts.push(
      browser
        ? ` ${escapeFormatString(String(rest.method))} ${escapeFormatString(String(rest.path))}`
  ...
  for (let i = 0; i < allEntries.length; i++) {
    ...
    if (browser) {
      ...
      console.log(
        `  %c${prefix}%c %c${escapeFormatString(entry.key)}:%c${val}`,
        cssColors.dim,
        cssColors.reset,
        cssColors.cyan,
        cssColors.reset,
      ...
      console.log(
        `  ${colors.dim}${prefix}${colors.reset} ${colors.cyan}${entry.key}:${colors.reset}${val}`,
      )
    }
    ...
    if (hasChildren && children) {
      ...
      for (let j = 0; j < children.length; j++) {
        ...
        if (browser) {
          console.log(
            `  %c${connector}  ${childPrefix}%c ${escapeFormatString(child)}`,
            cssColors.dim,
            cssColors.reset,
          )
        } else {
 ...
 * ```
 */
export function createLogger<T extends object = Record<string, unknown>>(
  initialContext: Record<string, unknown> = {},
  internalOptions?: CreateLoggerInternalOptions,
): AuditableLogger<T> {
  ...
  auditMethod.deny = function deny(
    reason: string,
  ...
  return {
    ...
    error(error: Error | string, errorContext?: FieldContext<T>): void {
      ...
      if (isPlainObject(context.error)) {
        mergeInto(context.error, errorObj)
      } else {
    ...
    info(message: string, infoContext?: FieldContext<T>): void {
      if (emitted) {
        const keys = infoContext
          ? [
              'message',
              ...Object.keys(infoContext as Record<string, unknown>).filter(
                (k) => k !== 'requestLogs',
              ),
            ]
          : ['message']
    ...
    warn(message: string, warnContext?: FieldContext<T>): void {
      if (emitted) {
        const keys = warnContext
          ? [
              'message',
              ...Object.keys(warnContext as Record<string, unknown>).filter(
                (k) => k !== 'requestLogs',
              ),
            ]
          : ['message']
 ...
 * ```
 */
export function createRequestLogger<T extends object = Record<string, unknown>>(
