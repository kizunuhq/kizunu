 ...
 */
function buildResourceAttributes(event: WideEvent, config: OTLPConfig): OTLPResource['attributes'] {
...
export function createOTLPDrain(overrides?: Partial<OTLPConfig>) {
  return defineHttpDrain<OTLPConfig>({
    ...
    resolve: async () => {
      ...
      if (!config.endpoint) {
        console.error(
          '[evlog/otlp] Missing endpoint. Set NUXT_OTLP_ENDPOINT or OTEL_EXPORTER_OTLP_ENDPOINT env var, or pass to createOTLPDrain()',
        )
        return null
      }
