...
### Testing Strategy → generate-tests

Whenever the workflow creates task test plans, writes tests, validates whether something needs tests,
or chooses between unit/integration/E2E coverage, **use the `generate-tests` skill if it is installed**.
`tlc-spec-driven` defines requirements and verification gates; `generate-tests` decides the right test
level using its thin/fat behavior classification. Do not mechanically turn every WHEN/THEN criterion
or every code layer into a dedicated unit/integration test when `generate-tests` classifies the code as
thin orchestration best covered by E2E behavior.
